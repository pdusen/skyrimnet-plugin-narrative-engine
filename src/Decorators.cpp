...
namespace NarrativeEngine::Decorators
{
    namespace
    {
        ...
        constexpr const char* kTensionDescription =
            "NarrativeEngine: most recent Director tension score, "
            "rendered as a decimal string in the range 0..100. "
            "Pre-first-evaluation default is 0. Ignores the actor argument.";

        constexpr const char* kPhaseDescription =
            "NarrativeEngine: current Freytag-pyramid phase name "
        ...
        void RegisterOne(const std::string& name, const std::string& description,
                         std::function<std::string(RE::Actor*)> callback)
        {
            ...
            if (SkyrimNetAPI::HasDecorator(name)) {
                logger::warn(
                    "Decorators::Register: '{}' is already registered "
