...
namespace NarrativeEngine::LetterPool
{
    namespace
    {
        ...
        {
            if (mood == "urgent" || mood == "menacing" || mood == "mournful")
                return 0.7f;
            if (mood == "warm")
                return 0.5f;
            return 0.3f;
        ...
        std::string BuildTagsJson(const std::string& topicTag, const std::vector<std::string>& facetTags)
        {
                             ...
                             const std::vector<std::string>& tags)
        {
            if (subjectFormId == 0 || contentText.empty()) {
                logger::warn("LetterPool::FireMemoryWrite[{}]: skipping — subjectFormId=0 "
                             "or empty content",
                             side);
                return;
            }
            if (!SkyrimNetAPI::IsAvailable() || !SkyrimNetAPI::IsMemorySystemReady()) {
                logger::warn("LetterPool::FireMemoryWrite[{}]: SkyrimNet or memory system "
            ...
            const auto subjUUID = SkyrimNetAPI::FormIDToUUID(static_cast<std::uint32_t>(subjectFormId));
            const auto counterUUID = SkyrimNetAPI::FormIDToUUID(static_cast<std::uint32_t>(counterpartFormId));
            logger::info("LetterPool::FireMemoryWrite[{}]: pre-write UUID snapshot "
            ...
            const int id = SkyrimNetAPI::AddMemory(static_cast<std::uint32_t>(subjectFormId),
                                                   contentText,
                                                   importance,
            ...
            if (id <= 0) {
                logger::warn("LetterPool::FireMemoryWrite[{}]: AddMemory returned {} "
            ...
            logger::info("LetterPool::FireMemoryWrite[{}]: memory written "
    ...
    void SetPerSlotQuests(const std::array<RE::TESQuest*, kPoolSize>& quests)
    {
        ...
        logger::info("LetterPool: per-slot quests bound ({} of {} slots dispatchable)", on, kPoolSize);
    }
    ...
    void Initialize()
    {
        ...
        for (std::size_t i = 0; i < kPoolSize; ++i) {
            ...
            if (!form) {
                ...
                ++failed;
                logger::error("LetterPool: EditorID '{}' did not resolve; slot {} disabled", edid, i);
                continue;
            }
            ...
            if (!book) {
                g_slots[i].bookFormID = 0;
                ++failed;
                logger::error("LetterPool: EditorID '{}' resolved to a non-BOOK form "
        ...
        g_initialized = true;
        logger::info("LetterPool: pool resolved ({} forms; {} failed)", resolved, failed);
    }
...

        {
            ...
            if (chosen == kPoolSize) {
                ...
                for (std::size_t i = 0; i < kPoolSize; ++i) {
                    ...
                    if (slot.state != State::Read || slot.bookFormID == 0 || slot.quest == nullptr)
                        continue;
            ...
            if (chosen == kPoolSize) {
                ...
                for (std::size_t i = 0; i < kPoolSize; ++i) {
                    const auto& slot = g_slots[i];
                    if (slot.state != State::InInventory || slot.bookFormID == 0 || slot.quest == nullptr)
                        continue;
                      ...
                      std::vector<std::string> tags)
    {
        ...
        RE::TESObjectBOOK* book = nullptr;
        {
            ...
            if (slot.bookFormID == 0) {
                logger::error("LetterPool::PopulateSlot: slot {} has no resolved FormID; aborting", slotIndex);
                return;
            }
        ...
        if (!book) {
            logger::error("LetterPool::PopulateSlot: slot {} FormID lookup failed at SetFullName", slotIndex);
            return;
        }
        ...
        logger::info("LetterPool: populated slot {} (book=0x{:08X}, sender='{}', body={} chars, topic='{}')",
                     slotIndex,
                     book->GetFormID(),
                     labelCopy,
    ...
    namespace
    {
        ...
        using GetDescription_t = void (*)(RE::TESDescription*, RE::BSString&, RE::TESForm*, std::uint32_t);
                                  ...
                                  std::uint32_t a_fieldType)
        {
            ...
            if (a_fieldType == kFieldType_DESC && a_parent && IsManagedForm(a_parent->GetFormID())) {
                std::string body;
        ...
        bool InstallHook(const char* name, std::uintptr_t targetAddr, void* detour, void** origOut)
        {
            const auto initStatus = MH_Initialize();
            if (initStatus != MH_OK && initStatus != MH_ERROR_ALREADY_INITIALIZED) {
                logger::error(
                    "LetterPool: MH_Initialize failed for {} hook (status={})", name, static_cast<int>(initStatus));
                return false;
            }

            const auto createStatus =
                MH_CreateHook(reinterpret_cast<LPVOID>(targetAddr), detour, reinterpret_cast<LPVOID*>(origOut));
            if (createStatus != MH_OK) {
                logger::error(
                    "LetterPool: MH_CreateHook failed for {} (status={})", name, static_cast<int>(createStatus));
                return false;
            }
            ...
            if (enableStatus != MH_OK) {
                logger::error(
                    "LetterPool: MH_EnableHook failed for {} (status={})", name, static_cast<int>(enableStatus));
                return false;
            }

            logger::info("LetterPool: {} hook installed (target=0x{:016X}, "
    ...
    namespace
    {
        struct BookMenuCloseSink : public RE::BSTEventSink<RE::MenuOpenCloseEvent>
        {
                                                  ...
                                                  RE::BSTEventSource<RE::MenuOpenCloseEvent>* /*src*/) override
            {
                ...
                // the next close.
                const int slotIdx = g_currentOpenSlot.exchange(-1, std::memory_order_acq_rel);
                if (slotIdx < 0) {
    ...
    void MarkDelivered(std::size_t slotIndex)
    {
        ...
        if (fireWrite) {
            ...
            const std::string playerName =
                (playerNameC && *playerNameC != '\0') ? std::string{playerNameC} : std::string{"the recipient"};
            const auto contentText = std::string{"I wrote and sent a letter to "} + playerName
                                     + ". The contents were as follows:\n\n" + body;
            ...
            FireMemoryWrite("sender", senderFormID, kPlayerFormID, contentText, mood, location, topicTag, tags);
        }
    ...
    {
        if (slotIndex >= kPoolSize) {
            logger::error("LetterPool::MarkDiscardedToContainer: slotIndex {} out of range", slotIndex);
            return;
        }
        logger::info("LetterPool: slot {} discarded to container (destFormID=0x{:08X})",
                     slotIndex,
        ...
        if (destination && bookFormID != 0) {
            ...
            if (book) {
                destination->RemoveItem(
                    book, std::numeric_limits<std::int32_t>::max(), RE::ITEM_REMOVE_REASON::kRemove, nullptr, nullptr);
            }
        }
    ...
    {
        if (slotIndex >= kPoolSize) {
            logger::error("LetterPool::MarkDroppedToCell: slotIndex {} out of range", slotIndex);
            return;
        }
        logger::info("LetterPool: slot {} dropped to cell (worldRefFormID=0x{:08X})",
                     slotIndex,
    ...
    namespace
    {
        ...
        struct ContainerChangedSink : public RE::BSTEventSink<RE::TESContainerChangedEvent>
        {
                                                  ...
                                                  RE::BSTEventSource<RE::TESContainerChangedEvent>* /*src*/) override
            {
                ...
                const auto playerID =
                    RE::PlayerCharacter::GetSingleton() ? RE::PlayerCharacter::GetSingleton()->GetFormID() : 0u;

                AsyncDispatch::EnqueueWork(
                    [slotIdx, oldContainer, newContainer, baseObj, playerID](const PluginThread::Token& pt) {
                        MainThread::FireAndForget(
                            pt, [slotIdx, oldContainer, newContainer, baseObj, playerID](const MainThread::Token&) {
                                // Any → player: courier delivery (or any other
                                // arrival into the player's inventory).
                                // MarkDelivered no-ops if the slot isn't
                                // PendingDelivery.
                                ...
                                // given, transferred).
                                if (oldContainer == playerID && newContainer != 0 && newContainer != playerID) {
                                    auto* form = RE::TESForm::LookupByID(newContainer);
                                    auto* ref = form ? form->AsReference() : nullptr;
                                    MarkDiscardedToContainer(static_cast<std::size_t>(slotIdx), ref);
                                    return;
                                }

                                // Player → world (drop). Look up the dropped
                                // REFR in the player's current cell so recycle
                                // can disable it.
                                if (oldContainer == playerID && newContainer == 0) {
                                    ...
                                    MarkDroppedToCell(static_cast<std::size_t>(slotIdx), worldRef);
                                    return;
                                }
    ...
    void RegisterContainerEventSink()
    {
        ...
        if (!src) {
            logger::error("LetterPool::RegisterContainerEventSink: ScriptEventSourceHolder unavailable");
    ...
    void OnLoad(SKSE::SerializationInterface* intfc, std::uint32_t version, std::uint32_t length)
    {
        ...
        if (version != kRecordVersion && version != 1) {
            logger::warn("LetterPool::OnLoad: unknown version {} (length={}); clearing pool", version, length);
            OnRevert();
            return;
        }
        ...
        if (slotCount != kPoolSize) {
            logger::warn("LetterPool::OnLoad: slot_count {} != current kPoolSize {}; clearing pool "
        ...
        for (std::size_t i = 0; i < kPoolSize; ++i) {
            ...
            if (!ReadString(intfc, s.body) || !ReadString(intfc, s.senderLabel) || !ReadString(intfc, s.topicTag)) {
            ...
            if (version >= 2) {
                ...
                for (std::uint16_t t = 0; t < tagCount; ++t) {
                    ...
                    if (!ReadString(intfc, tag)) {
                        logger::error("LetterPool::OnLoad: short read on slot {} tag {}", i, t);
                        OnRevert();
                        return;
                    }
                ...
                case State::Read: {
                    ...
                    if (count <= 0) {
                        DemoteToFreeLocked(i,
                                           slot.state == State::InInventory ? "InInventory but player has 0 copies"
        ...
        if (!pendingDeliveryOrphans.empty() || !freeSlotBooks.empty()) {
            ...
            AsyncDispatch::EnqueueWork([orphans = std::move(pendingDeliveryOrphans),
                                        freeBooks = std::move(freeSlotBooks)](const PluginThread::Token& pt) mutable {
                MainThread::FireAndForget(
                    pt, [orphans = std::move(orphans), freeBooks = std::move(freeBooks)](const MainThread::Token&) {
                        ...
                        for (const auto& [i, bookFormID] : freeBooks) {
                            ...
                            if (courierContainer) {
                                ...
                                auto inv = courierContainer->GetInventory(
                                ...
                                if (invIt != inv.end() && invIt->second.first > 0) {
                                    ...
                                    if (courierQuest) {
                                        for (auto* ref : refsToRelease) {
                                            ...
                                            const bool queued =
                                                QuestUtils::VMDispatchOnQuest(courierQuest,
                                                                              "WICourierScript"sv,
