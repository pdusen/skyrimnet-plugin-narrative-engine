...
namespace NarrativeEngine::BeatParamHelpers
{
        ...
        std::string*          failureReason)
    {
        ...
        if (!parameters.is_object()) return fail("parameters_not_object");
        ...
        if (formID == 0) return fail("sender_npc_form_id_zero");
        return formID;
    }

    UrgencyHint ParseUrgencyHint(const nlohmann::json& parameters)
    {
        if (!parameters.is_object()) return UrgencyHint::Medium;
        auto it = parameters.find("urgency_hint");
        if (it == parameters.end() || !it->is_string()) {
            return UrgencyHint::Medium;
        }
        const auto v = it->get<std::string>();
        if (v == "low")  return UrgencyHint::Low;
        if (v == "high") return UrgencyHint::High;
        return UrgencyHint::Medium;
    }
        ...
        std::string* failureReason)
    {
        ...
        if (!sender)              return fail("sender_no_longer_resolves");
        if (sender->IsDead())     return fail("sender_died_during_compose");
        if (sender->IsDisabled()) return fail("sender_disabled_during_compose");
        return sender;
    }
}
