...
namespace NarrativeEngine
{
    ...
    namespace
    {
        ...
        // Optional in-fiction motivation for this beat, generated by
        // the action-select LLM. Threaded into the compose prompt as
        // the authoritative topic seed — with this in place the
        // compose LLM works from a Director-picked subject rather
        // than inferring one from the memory tail. Empty when the
        // action-select response omitted the field.
    ...
    void NPCLetterBeat::OnStart(const BeatContext& /*ctx*/, const nlohmann::json& parameters)
    {
        ...
        // Mirrors the visit beat's handling of the same field.
        std::string justification;
