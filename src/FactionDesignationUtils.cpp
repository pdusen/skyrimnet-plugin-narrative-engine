...
namespace NarrativeEngine::FactionDesignationUtils
{
                              ...
                              std::string_view logTag)
    {
        ...
        const auto walk = [&](const RE::BSTArray<RE::ActorHandle>& list) {
            for (const auto& h : list) {
                ...
                if (actor->GetFactionRank(fact, false) >= designatedRank) {
                    ...
                    ++swept;
                    logger::info("{}[FACTION]: swept stale rank-{}+ member 0x{:08X} "
                                 "-> rank {}",
                                 logTag,
        ...
        if (swept > 0) {
            logger::info("{}[FACTION]: pre-dispatch sweep demoted {} stale rank-{}+ actor(s)",
                         logTag,
                         swept,
                         static_cast<int>(designatedRank));
        }
                             ...
                             std::string_view logTag)
    {
        ...
        if (currentRank == designatedRank) {
            logger::info("{}[FACTION]: sender 0x{:08X} already at rank {}; no "
                         "promotion needed",
                         logTag,
                         sender->GetFormID(),
                         static_cast<int>(designatedRank));
        ...
        if (currentRank < 0) {
            logger::info("{}[FACTION]: added sender 0x{:08X} to faction at rank {}",
                         logTag,
                         sender->GetFormID(),
                         static_cast<int>(designatedRank));
        } else {
            logger::info("{}[FACTION]: promoted sender 0x{:08X} from rank {} to rank {}",
                         logTag,
                         sender->GetFormID(),
                         currentRank,
                         static_cast<int>(designatedRank));
    ...
    void DemoteToCandidate(RE::TESFaction* fact, RE::Actor* sender, std::int8_t candidateRank, std::string_view logTag)
    {
        ...
        logger::info("{}[FACTION]: demoted sender 0x{:08X} from rank {} back to rank {}",
                     logTag,
                     sender->GetFormID(),
                     currentRank,
                     static_cast<int>(candidateRank));
